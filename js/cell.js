class Cell{
  constructor(x, y, w, h, state, i, j, special){
    ...
    if(this.state == "bar"){
      if(this.special){
        ...
        switch(this.i){
          case 0: this.value = Math.floor(Math.random() * (10 - 1) + 1); break;
          case 1: this.value = Math.floor(Math.random() * (20 - 11) + 11); break;
          case 2: this.value = Math.floor(Math.random() * (30 - 21) + 21); break;
          case 3: this.value = Math.floor(Math.random() * (40 - 31) + 31); break;
        }
      }
  ...
  draw(){
    if(!this.matched || this.state == "points")
    {
      ...
      if(this.state != "bar") ctx.drawImage(cellUnmatchedImg, this.x, this.y, this.w*2, this.h*2);
      ...
      if(this.state != "points"){
        if(this.state == "bar" && (this.value == "W" || this.value == "SW" || this.value == "G")){
          switch(this.value){
            case "W": ctx.drawImage(powerWImg, this.x + this.w/2, this.y + this.h/2, this.w, this.h); break;
            case "SW": ctx.drawImage(powerSWImg, this.x + this.w/2, this.y + this.h/2, this.w, this.h); break;
            case "G": ctx.drawImage(powerGImg, this.x + this.w/2, this.y + this.h/2, this.w, this.h); break;
          }
        }else{
